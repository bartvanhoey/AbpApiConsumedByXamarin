...
namespace XamarinBookStoreApp.Services.Http
{
    public class HttpClientService<T, C> : IHttpClientService<T, C> where T : class where C : class
    {
        ...
        public async Task<GetResult<T>> GetAsync(string uri)
        {
            //var accessToken = await GetAccessTokenAsync();

            //_httpClient = new Lazy<HttpClient>(() => new HttpClient(GetHttpClientHandler()));
            ...
            var result = JsonConvert.DeserializeObject<GetResult<T>>(content);
