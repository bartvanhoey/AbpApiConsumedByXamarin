...
namespace AbpApi.IdentityServer
{
    public class IdentityServerDataSeedContributor : IDataSeedContributor, ITransientDependency
    {
        ...
        {
            await CreateApiScopeAsync("AbpApi");
        }
        ...
        private async Task CreateApiResourcesAsync()
        {
            ...
            await CreateApiResourceAsync("AbpApi", commonApiUserClaims);
        }
        ...
        private async Task CreateClientsAsync()
        {
            ...
            var consoleAndAngularClientId = configurationSection["AbpApi_App:ClientId"];
            if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
            {
                var webClientRootUrl = configurationSection["AbpApi_App:RootUrl"]?.TrimEnd('/');

                await CreateClientAsync(
                    name: consoleAndAngularClientId,
                    scopes: commonScopes,
                    grantTypes: new[] { "password", "client_credentials", "authorization_code" },
                    secret: (configurationSection["AbpApi_App:ClientSecret"] ?? "1q2w3e*").Sha256(),
            ...
            {
                var blazorRootUrl = configurationSection["AbpApi_Blazor:RootUrl"].TrimEnd('/');

                await CreateClientAsync(
                    name: blazorClientId,
                    scopes: commonScopes,
                    grantTypes: new[] { "authorization_code" },
                    secret: configurationSection["AbpApi_Blazor:ClientSecret"]?.Sha256(),
                    requireClientSecret: false,
            ...
            {
                var swaggerRootUrl = configurationSection["AbpApi_Swagger:RootUrl"].TrimEnd('/');

                await CreateClientAsync(
                    name: swaggerClientId,
                    scopes: commonScopes,
                    grantTypes: new[] { "authorization_code" },
                    secret: configurationSection["AbpApi_Swagger:ClientSecret"]?.Sha256(),
                    requireClientSecret: false,
